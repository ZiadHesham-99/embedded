...
void GPIO_voidSetPinValue(PORT_NAME local_u8PortID, PIN_NUM local_u8PinID, LEVEL local_u8Value){
	 switch(local_u8PortID){
	        case PORT_NAME_A:
	            if(local_u8Value == LEVEL_LOW )        GPIOA->BSRR=(local_u8Value<<(local_u8PinID+16));
	            else                     			   GPIOA->BSRR=(local_u8Value<<local_u8PinID);
	            break;
	        case PORT_NAME_B:
	            if(local_u8Value == LEVEL_LOW )        GPIOB->BSRR=(local_u8Value<<(local_u8PinID+16));
	            else                      			   GPIOB->BSRR=(local_u8Value<<local_u8PinID);
	            break;
	        case PORT_NAME_C:
	            if(local_u8Value == LEVEL_LOW )        GPIOC->BSRR=(local_u8Value<<(local_u8PinID+16));
	            else                        		   GPIOC->BSRR=(local_u8Value<<local_u8PinID);
	            break;
	    }
}
