...
void GPIO_setPinMode(void){
	for(int i = 0 ; i<NUMBER_OF_USED_PINS ; i++){
		...
		case PORT_NAME_A:{
			...
			if(ARRAY_OF_PINS[i].MODE==AF){
				if(ARRAY_OF_PINS[i].PIN < 8){
					...
					GPIOA->AFRL |= (ARRAY_OF_PINS[i].AF_ID << ARRAY_OF_PINS[i].PIN *4 );
				}
				else{
					GPIOA->AFRH &= ~ (0b1111<(ARRAY_OF_PINS[i].PIN -8)*4);
					GPIOA->AFRH |= (ARRAY_OF_PINS[i].AF_ID << (ARRAY_OF_PINS[i].PIN-8 )*4 );
				}
			}
			else{
		...
		case PORT_NAME_B:{
			...
			if(ARRAY_OF_PINS[i].MODE==AF){
				if(ARRAY_OF_PINS[i].PIN < 8){
					GPIOB->AFRL &= ~ (0b1111<ARRAY_OF_PINS[i].PIN *4);
					GPIOB->AFRL |= (ARRAY_OF_PINS[i].AF_ID << ARRAY_OF_PINS[i].PIN *4 );
				}
				else{
					GPIOB->AFRH &= ~ (0b1111<(ARRAY_OF_PINS[i].PIN -8)*4);
					GPIOB->AFRH |= (ARRAY_OF_PINS[i].AF_ID << (ARRAY_OF_PINS[i].PIN-8 )*4 );
				}
			}
			else{
				if(ARRAY_OF_PINS[i].TYPE != NOT_USED){
		...
		case PORT_NAME_C:{
			...
			if(ARRAY_OF_PINS[i].MODE==AF){
				if(ARRAY_OF_PINS[i].PIN < 8){
					GPIOC->AFRL &= ~ (0b1111<ARRAY_OF_PINS[i].PIN *4);
					GPIOC->AFRL |= (ARRAY_OF_PINS[i].AF_ID << ARRAY_OF_PINS[i].PIN *4 );
				}
				else{
					GPIOC->AFRH &= ~ (0b1111<(ARRAY_OF_PINS[i].PIN -8)*4);
					GPIOC->AFRH |= (ARRAY_OF_PINS[i].AF_ID << (ARRAY_OF_PINS[i].PIN-8 )*4 );
				}
			}
			else{
				if(ARRAY_OF_PINS[i].TYPE != NOT_USED){
